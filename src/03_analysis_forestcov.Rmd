...
```{r, warning=F, message=F, fig.width = 10, fig.height = 6}
# plot
ggplot(data = rx_crown_sum %>% dplyr::filter(unit == (rx_fire %>% dplyr::slice(3))$unit)) + 
  geom_col(aes(y = crown_group_class, x = area_ha, fill = crown_group_class), width = 0.7) +
  scale_fill_manual(values = scale_fill_manual_values_fill)
  geom_text(
    aes(y = crown_group_class, x = area_ha + 2, label = scales::comma(area_ha, accuracy = 1))
    , color = "black"
    , size = 4
    , fontface = "bold"
    # , position = position_dodge(0.9)
