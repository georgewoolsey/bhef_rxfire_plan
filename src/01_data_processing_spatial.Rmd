...
kable(stem_map %>% sf::st_set_geometry(NULL) %>% 
    ...
    dplyr::arrange(unit) 
  , format = "html" 
  , caption = "Tree Measurement Descriptive Statistics by Unit"
  , digits = 2
) %>% 
...
### FACTS Harvest Map with Research Plots
...
mapview(research_plots
        , zcol = "trt"
        , col.regions = viridis::turbo(n=length(unique(research_plots$trt)))
        , lwd = 2
        , alpha.regions = 0.8
        , label = c("trt")
        , legend = FALSE
...
research_plots_harvests <- sf::st_intersection(
  ...
  , bhef_harvests_l15 %>% 
      ...
      dplyr::filter(!treatment_type_grp %in% c("Sanitation Cut")) %>% 
      dplyr::rename_with(~ paste0("harvest_", .), -geometry)
)

view(research_plots %>% st_set_geometry(NULL) %>% arrange(plot))
view(research_plots_harvests %>% st_set_geometry(NULL) %>% arrange(plot))
...
mapview(bhef_harvests_l15
  ...
  mapview(stem_map
    , zcol = "unit"
    , col.regions = viridis::turbo(n=length(unique(stem_map$unit)), direction = -1, alpha = 0.8)
