...
See [Zhen et al. (2014)](https://www.mdpi.com/2072-4292/6/1/555/pdf-vor) for variable window size in ITD.

```{r, warning=F, message=F, include=FALSE, eval=FALSE}
...
# local maximum filtering (LMF) with variable window size
  ws_fn <- function(x) {x * 0.1 + 3}
  ttops <- lidR::locate_trees(nlas, lmf(ws = ws_fn)) %>% 
    sf::st_geometry() %>% 
    sf::st_transform(crs = sf::st_crs(bhef_boundary))
  ...
  ggplot() +
    stars::geom_stars(data = chm) +
    geom_sf(data = ttops, color = viridis::viridis(n=1, direction = -1), size = 0.5) +
    scale_fill_viridis_c(option = "mako", alpha = 0.9) +
    labs(
      title = "BHEF Canopy Height Model"
      , subtitle = sf::st_crs(chm)$input
