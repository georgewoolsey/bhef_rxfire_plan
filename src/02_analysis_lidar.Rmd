...
######################################################
  chm <- lidR::rasterize_canopy(
    ...
    chm_smooth <- lidR::rasterize_canopy(
      nlas
      , res = 1
      , p2r(subcircle = 0.0
            # , na.fill = tin()
    ...
    kernel <- matrix(1,3,3)
    chm_smooth <- terra::focal(chm_smooth, w = kernel, fun = median, na.rm = TRUE) %>% 
      stars::st_as_stars() %>%
      stars::st_warp(crs = sf::st_crs(bhef_boundary))
...
# local maximum filtering (LMF) with variable window size
  ...
  ggplot() +
    stars::geom_stars(data = chm) +
    geom_sf(data = ttops, color = viridis::viridis(n=1, direction = -1), alpha = 0.6, shape = ".") +
    scale_fill_viridis_c(option = "mako", alpha = 0.9) +
    labs(
      title = "BHEF Canopy Height Model with Tree Tops"
      , subtitle = sf::st_crs(chm)$input
    ) +
    theme_bw() +
    theme(
