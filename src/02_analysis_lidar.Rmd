...
# implements an algorithm for tree segmentation based on 
  ...
  algo <- lidR::dalponte2016(chm = chm_smooth, treetops = tree_tops, th_tree = 2)
  crowns_st <- algo()
# quick plot
...
# plot
  ggplot() +
    geom_sf(data = crowns_group, aes(fill = crown_group_class), lwd = 0) +
    scale_fill_viridis_d(option = "turbo", alpha = 0.9) + 
    labs(
      title = "BHEF Crown Groups"
...
```{r, warning=F, message=F, fig.width = 10, fig.height = 6}
# save cleaned data for reading to R later
sf::st_write(chm_smooth, "../data/chm_smooth.gpkg", append = FALSE)
