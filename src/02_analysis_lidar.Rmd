...
ttops %>% sf::st_set_geometry(NULL) %>% 
  ...
  ggplot(.) +
    ...
    scale_x_continuous(breaks = seq(min(ttops$Z), max(ttops$Z)+1, 1))+
...
# implements an algorithm for tree segmentation based on 
  ...
  algo <- lidR::dalponte2016(chm = chm_smooth, treetops = ttops, th_tree = 2)
  crowns_st <- algo()
# quick plot
...
# join back to individual tree crowns
  crowns_group_intersect <- unlist(sf::st_intersects(crowns_sf, crowns_group))
  

  
# if you do want to construct the geometry of the clusters, use st_union:
...
```{r, warning=F, message=FALSE, eval=FALSE, include=FALSE, echo=FALSE}
